				...
				<div class="topicones">
				    <%
			            ...
			            if(sWorkTimeMessage.length() > 0){
			                %>
                                        <img style="float:right;" height='22' src="<c:url value='/_img/themes/default/men_at_work.gif'/>" alt="<%=(getTranNoLink("Web.Occup","medwan.common.workuntil",sWebLanguage)+" "+sWorkTimeMessage)%>"/><%
			            }
		                ...
		                if(sTmpPersonid.length() > 0){
		                    ...
		                    if(!bFingerPrint){
		                        %> <a class="imglink" href="javascript:enrollFingerPrint();">
                                            <img border='0' height='22' src="<c:url value='/_img/themes/default/fingerprint.png'/>" alt="<%=getTranNoLink("web","enrollFingerPrint",sWebLanguage)%>"/></a><%
		                    }
		                    if(!bBarcode){
		                        %> <a class="imglink" href="javascript:printPatientCard();">
                                            <img border='0' height='22' src="<c:url value='/_img/themes/default/badge.png'/>" alt="<%=getTranNoLink("web","printPatientCard",sWebLanguage)%>"/></a><%
		                    }
		                    if(!bPicture){
		                        %> <a class="imglink" href="javascript:storePicture();">
                                            <img border='0' height='22' src="<c:url value='/_img/themes/default/camera.png'/>" alt="<%=getTranNoLink("web","loadPicture",sWebLanguage)%>"/></a><%
		                    }
		                }
		            %>
				</div>
